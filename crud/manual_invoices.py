...
def get_manual_invoices(db: Session, skip: int = 0, limit: int = 100):
...
def update_manual_invoices(
    ...
    if db_manual_invoice:
        ...
        db_manual_invoice.invoice_amount = manualInvoice.invoice_amount
        db.commit()
        db.refresh(db_manual_invoice)
    return db_manual_invoice


def delete_manual_invoices(db: Session, manual_invoice_id: int):
    db_manual_invoice = (
        db.query(ManualInvoice)
        .filter(ManualInvoice.manual_invoice_id == manual_invoice_id)
        .first()
